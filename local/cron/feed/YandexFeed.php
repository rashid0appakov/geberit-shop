...
$_SERVER["DOCUMENT_ROOT"] = "/var/www/clients/client0/web1/web/"; // /home/duravit/www/drvt.shop/";
...
class YandexFeed
{
    ...
    private function WriteDeliveryOptions($h)
    {
        $date=date('w H i');
        $arDate=explode(' ',$date);
        $day=$arDate[0];
        $hour=$arDate[1];
        $minute=$arDate[2];
        if (( $day == 5 && $hour >= 16 ) || ( $day == 6 && $hour < 21 )){
            $delivery_days='2-4';
        }
        else
        {
        ...
        fwrite($h, "\t\t<delivery-options>" . PHP_EOL);
        fwrite($h, "\t\t\t<option cost=\"390\" days=\"". $delivery_days ."\" />" . PHP_EOL);
        fwrite($h, "\t\t</delivery-options>" . PHP_EOL);
    }
    ...
    private function WriteFileOffers($h)
    {
        ...
        foreach ($this->data['elements'] as $elementId => $element)
        {
           ...
           // fwrite($h, "<!-------". $vendor . "\" available=\"" . $this->SITE_SERVER_NAME . "----->" . PHP_EOL);
            ...
            // output
            //fwrite($h,json_encode($element, JSON_UNESCAPED_UNICODE));
            ...
            $ar_props = explode('; ',$element['description']);
            foreach($ar_props as $prop){
                //fwrite($h,json_encode($prop, JSON_UNESCAPED_UNICODE));
                $ar_prop=explode(': ',$prop);
                if(strlen($ar_prop[1])>0)
                    fwrite($h, "\t\t\t\t<param name=\"". $ar_prop[0] ."\">" . $ar_prop[1] . "</param>" . PHP_EOL);
            }
