...
$arSelect = Array("ID", "IBLOCK_ID", "NAME", "ARTNUMBER", "ARTNUMBER", "DETAIL_PAGE_URL","QUANTITY","CATALOG_GROUP_1");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
...
$dataExport = array (
	array('ID','LINK','ARTICLE','PRICE','VAT','STOCK','DELIVERY_DAYS'),
);
echo "<pre>";
$dataExport = $dataExport + $dataExport;
print_r($dataExport);

$arTechProps=array("SYS_UNDER_THE_ORDER","LAST_ID","TIME_BUY","ADDITIONAL","FILES_DOCS","MORE_PHOTO","NEWPRODUCT","DISCOUNT","COUNTRY","SALELEADER","GUARANTEE","SHOWROOM","PO_KOMU","PO_KOMU_EKB","PO_KOMU_SPB","SALEGOODS","DISCONTINUED","COLLECTION_ITEMS","PRICE_UPDATE","PRICE_UPDATE_SPB","KOMLP_CACHE","ADDITIONAL_CACHE","PRICE_UPDATE_EKB");
$arArrayProps=array("TSVET","MORE_PHOTO","COUNTRY","MATERIAL","KOLLEKTSIYA2","RODINA_BRENDA","GARANTIYA_PROIZVODITELYA","FORMA","MONTAZH");
...
while($ob = $res->GetNextElement()){ 
 ...
 foreach($arProps as $v){
 	if(!!$v["VALUE"] && !in_array($v["CODE"],$arTechProps))
 	{
...
$csv .= "\r\n";
foreach($arResult as $code=>$arItem)
{
    $url="https://shop-jd.ru/".$arItem["DETAIL_PAGE_URL"];
    $csv .= $arItem["NAME"].";".$url.";".$arItem["QUANTITY"].";".$arItem["CATALOG_PRICE_1"].";";

    foreach($colTitleCodes as $k=>$code)
    {
        if($k<4)
            continue;

        $csv .= $arItem["PROPERTIES"][$code]["VALUE"].";";
    }   
    $csv .= "\r\n";
}
//die();
