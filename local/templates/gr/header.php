...
<head>
	...
	<meta name="yandex-verification" content="a16029ca3c515171" />
</head>
<body data-section="<?$APPLICATION->ShowProperty('currentSectionId', '0');?>" data-element="<?$APPLICATION->ShowProperty('currentElementId', '0');?>">
	...
	<?if (IS_SALE) {?>
	<!-- Sale !-->
	<link rel="stylesheet" type="text/css" href="/local/templates/.default/sale.css">
	<div class="notify-wrapper notify-wrapper--top"><div id="campaign-christmas-landing" class="notification notify notify--banner notification--static notification--no-shadow notification--no-border notification--hide-disable mg-none-i notification--show">
			<div class="notification__content container">
				<a  сlass="nostyle medium">
					<p class="mg-none">Успей купить по выгодной цене!  </p>
					<span class="button button--xs button--flat button--inverted mg-left-lv2">В связи с повышением курса евро, цены будут пересчитаны с 20.03.2020 г.</span>
				</a>
				<span class="santa"></span>
			</div>
		</div>
			...
			<div class="columns is-mobile">
				...
				<div class="phone column custom-live-ajax-update" id="custom-live-ajax-update-header-phone2">
					...
					if(empty($_SERVER['HTTP_IS_SUB_HEADER'])){
						?>
						<span class="call_phone_1"><a class="phone" href="tel:<?=$arContact['ADD_PHONE']['NUMBER']?>"><?=$arContact['ADD_PHONE']['VALUE']?></a></span>
						<?
					}
					else{
						?>
		...
		<div class="container is-widescreen">
			<div class="columns is-mobile">
				...
				<div class="phone column is-2 custom-live-ajax-update" id="custom-live-ajax-update-header-phone" style="margin-top: -35px;">
					...
					if(empty($_SERVER['HTTP_IS_SUB_HEADER'])){
						?>
						<span class="call_phone_1"><a href="tel:<?=$arContact['ADD_PHONE']['NUMBER']?>" class="call"><?=$arContact['ADD_PHONE']['VALUE']?></a></span>
						<?
					}
					else{
						?>
	...
	<div id="bottom-header" class="bottom-header hero">
		<div class="container is-widescreen">
			<div class="columns is-mobile">
				...
				if(empty($_SERVER['HTTP_IS_SUB_HEADER'])){
					?>
					<span class="m_call_phone_1"><a href="tel:<?=$arContact['ADD_PHONE']['NUMBER']?>" class="mobile-phone-button search bottom-header__search _popup-catalog-menu-start-mobile-button--bottom-header"
				...
				<div class="phone column">
					...
					if(empty($_SERVER['HTTP_IS_SUB_HEADER'])){
						?>
						<span class="m_call_phone_1"><a class="phone" href="tel:<?=$arContact['ADD_PHONE']['NUMBER']?>"><?=$arContact['ADD_PHONE']['VALUE']?></a></span>
						<?
					}
					else{
						?>
						<span class="m_call_phone_1"><a class="phone" href="tel:<?=$arContact['PHONE']['NUMBER']?>"><?=$arContact['PHONE']['VALUE']?></a></span>
						<?
					}
