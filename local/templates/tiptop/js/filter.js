        ...
        setTimeout(function () {
            $.ajax({
                ...
                success: function(data) {
					var URL, matches, select_brands, type_select_brands, filter_url;
					
					matches = data.FILTER_URL.match(/manufacturer-is-([a-z0-9\-\_]+)\//);
					
					console.log(matches);
					
					if(matches !== null){
						select_brands = matches[1];
						type_select_brands = select_brands.match(/\-or\-/);

						if(type_select_brands === null){
							filter_url = data.FILTER_URL.replace(matches[0], '');
							
							URL = 'https://' + select_brands + '.tiptop-shop.ru' + filter_url;
						}
						else {
							URL = 'https://tiptop-shop.ru' + data.FILTER_URL;
						}
					}
					else {
						URL = 'https://tiptop-shop.ru' + data.FILTER_URL;
                    ...
                    _filter_form.find('input[name=current_url]').val(URL);
                    _filter_form.attr('action', URL);
        ...
        _filter_form.find('input[type=checkbox], select').change(function(){
            if ($(this).hasClass('dropdown-open'))
            {
            	...
            	else
            	{
            		...
            		$('.filter-fast-search-checkbox-'+$(this).data('key')+' input').focus();
            	}
                return true;
