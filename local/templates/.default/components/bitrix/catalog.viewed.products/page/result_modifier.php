...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$arItem["ITEM_PRICES"][0]["PRICE"] = $arItem["MIN_PRICE"]["VALUE"];
		$arItem["ITEM_PRICES"][0]["BASE_PRICE"] = $arItem["MIN_PRICE"]["DISCOUNT_VALUE"];
		$arItem["ITEM_PRICES"][0]["PRINT_PRICE"] = $arItem["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"];
		$arItem["ITEM_PRICES"][0]["PRINT_BASE_PRICE"] = $arItem["MIN_PRICE"]["PRINT_VALUE"];
		
		$arFile = CFile::GetFileArray($arItem["DETAIL_PICTURE"]["ID"]);
		$arItem["DETAIL_PICTURE"]["OTHER"] = [
	...
	$arSections = array_count_values($arSectionIds);
	foreach($arSections as $sectionId => $sectionCount){
		$arResSection[$sectionId]["ID"] = $sectionId;
		$arResSection[$sectionId]["COUNT"] = $sectionCount;
		
		$arSectionIdsUni[] = $sectionId;
	}
	
	$rsSect = CIBlockSection::GetList(array(), array('IBLOCK_ID' => CATALOG_IBLOCK_ID, 'ID' => $arSectionIdsUni));
	while ($arSect = $rsSect->GetNext())
	{
		$arResSection[$arSect['ID']]['NAME'] = $arSect['NAME'];
	}

	$arNewItemsList[$key]['LAST_ELEMENT'] = 'Y';
	$arResult['ITEMS'] = $arNewItemsList;
	$arResult['SECTIONS'] = $arResSection;
