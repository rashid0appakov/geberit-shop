...
$arParams['DETAIL_PROPERTY_CODE'] = getShowPropertyValues($arParams["IBLOCK_ID"], $arParams["CACHE_TIME"]);
...
if (intval($arResult["VARIABLES"]["ELEMENT_ID"]) > 0)
{
...
$cacheDir = "/catalog/element";
$obCache = new CPHPCache();
if ($obCache->InitCache(false, $cacheId, $cacheDir))
{
	  $currentElement = $obCache->GetVars();
}
elseif ($obCache->StartDataCache())
{
	...
	if ($arElement = $ob->Fetch())
	{
		$currentElement = array();

		$currentElement["ID"] = $arElement["ID"];
		$currentElement["NAME"] = $arElement["NAME"];
		$currentElement["IBLOCK_SECTION_ID"] = $arElement["IBLOCK_SECTION_ID"];

		$obCache->EndDataCache($currentElement);
	}
	else
	{
...
$arResult['VARIABLES']['ELEMENT_ID'] = $currentElement["ID"];
$arResult['VARIABLES']['SECTION_ID'] = $currentElement["IBLOCK_SECTION_ID"];
$APPLICATION->SetPageProperty('currentElementId', $currentElement["ID"]);
$APPLICATION->SetPageProperty('currentSectionId', $currentElement["IBLOCK_SECTION_ID"]);

$ob = CIBlockElement::GetElementGroups($currentElement["ID"], true);
while($ar_group = $ob->Fetch()) {
    if($ar_group["IBLOCK_SECTION_ID"]!=$currentElement["IBLOCK_SECTION_ID"]){ continue; }
    $currentElement["SECTION_ID"] = $ar_group["ID"];
    break;
}
	...
	<div class="container goods__container">
			...
			<ul class="breadcrumbs">
				<?$APPLICATION->IncludeComponent(
					...
					array(
						...
						"IBLOCK_ID" => $arParams["IBLOCK_ID"],
						"SECTION_ID" => $currentElement["SECTION_ID"]?:$currentElement["IBLOCK_SECTION_ID"],
		...
		<div>
			...
			<?$componentElementParams = array(
				...
				"MAIN_SITE_BRAND" => ($bIsBrandSite ? MAIN_SITE_BRAND : false),
				'IBLOCK_TYPE' => $arParams['IBLOCK_TYPE'],
				'IBLOCK_ID' => $arParams['IBLOCK_ID'],
				'PROPERTY_CODE' => [
					'OLD_ID'
				],
				'CUSTOM_PROPERTY_CODE' => $arParams['DETAIL_PROPERTY_CODE'],
				...
				'DISPLAY_INSTALLMENT_PLAN'  => $arParams['DISPLAY_INSTALLMENT_PLAN']
			);
...
/*if ($USER->IsAdmin())
	...
	$title = '';
	$arSelect = Array("ID", "NAME", "IBLOCK_ID","PROPERTY_MANUFACTURER","PROPERTY_SERIES","PROPERTY_ARTNUMBER","DETAIL_PICTURE","TIMESTAMP_X");
	...
	while($ob = $res->GetNextElement())
	{
	 ...
	 if($arFields['PROPERTY_MANUFACTURER_VALUE']>0){
	 	$res_m = CIBlockElement::GetByID($arFields['PROPERTY_MANUFACTURER_VALUE']);
		if($ar_res = $res_m->GetNext()){
		  $title .= $ar_res['NAME'].' ';
		  $MANUFACTURER = $ar_res['NAME'];
		}
	 }
	 if($arFields['PROPERTY_SERIES_VALUE']>0){
	 	$res_m = CIBlockElement::GetByID($arFields['PROPERTY_SERIES_VALUE']);
		if($ar_res = $res_m->GetNext())
		  $title .= $ar_res['NAME'].' ';
	 }
	  if(strlen($arFields['PROPERTY_ARTNUMBER_VALUE'])>0){
		$title .= $arFields['PROPERTY_ARTNUMBER_VALUE'].' ';
	 }
	 $nameEl= $arFields['NAME'];
	  $OGPic = CFile::GetPath($arFields["DETAIL_PICTURE"]);
	}

	$geo_id = $APPLICATION->get_cookie("GEOLOCATION_ID");
	$geo = GetGeoText();
	$title = $nameEl.' купить '.$geo.', цена на geberit-shop.ru';

	if ($geo_id == 817 || strpos($_SERVER["SERVER_NAME"], 'spb.')!==false ){
		$descr = 'Купить ✅ '.$nameEl.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой по Санкт-Петербургу — низкие цены, ГАРАНТИЯ от производителя!';
		// $descr = '❤️ '.$nameEl.' по низкой цене у официального дилера ✅ Geberit, доставка по Санкт-Петербургу и РФ.';
	}elseif ($geo_id == 2201 || strpos($_SERVER["SERVER_NAME"], 'ekb.')!==false ){
		$descr = 'Купить ✅ '.$nameEl.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой по Екатегинбургу — низкие цены, ГАРАНТИЯ от производителя!';
		//$descr = '❤️ '.$nameEl.' по низкой цене у официального дилера ✅ Geberit, доставка по Екатегинбургу и РФ.';
	}elseif ($geo_id == 1095 || strpos($_SERVER["SERVER_NAME"], 'krasnodar.')!==false ){
		$descr = 'Купить ✅ '.$nameEl.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой по Краснодару — низкие цены, ГАРАНТИЯ от производителя!';
		//$descr = '❤️ '.$nameEl.' по низкой цене у официального дилера ✅ Geberit, доставка по Краснодару и РФ.';
	}elseif ($geo_id == 2622 || strpos($_SERVER["SERVER_NAME"], 'novosibirsk.')!==false ){
		$descr = 'Купить ✅ '.$nameEl.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой по Новосибирску — низкие цены, ГАРАНТИЯ от производителя!';
		//$descr = '❤️ '.$nameEl.' по низкой цене у официального дилера ✅ Geberit, доставка по Новосибирску и РФ.';
	}else{
		$descr = 'Купить ✅ '.$nameEl.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой по Москве — низкие цены, ГАРАНТИЯ от производителя!';
		//$descr = '❤️ '.$nameEl.' по низкой цене у официального дилера ✅ Geberit, доставка по Москве и РФ.';
	}

	if(true || $GLOBALS['USER']->IsAdmin()){
		preg_match('/(.*Geberit[0-9a-zA-Z\/\s-]+)/i',$arFields['NAME'],$matches);
		if($matches[1]){ $nameEl = trim($matches[1]); }
		//$APPLICATION->SetTitle($nameEl, false);
		$title = $nameEl.' купить '.$geo.', цена на geberit-shop.ru';
		$descr = $nameEl.' ✅ купить '.$geo.' в интернет-магазине Geberit по низкой цене с монтажом ⭐ и доставкой — низкие цены, ГАРАНТИЯ от производителя!';
	}
...
$APPLICATION->AddHeadString('<meta property="og:type" content="product.item">',true);
...
$APPLICATION->AddHeadString('<meta property="product:brand" content="'.$MANUFACTURER.'">',true);
$APPLICATION->AddHeadString('<meta property="og:updated_time" content="'.$TIMESTAMP_X.'">',true);
