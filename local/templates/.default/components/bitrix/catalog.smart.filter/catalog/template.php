...
<?if(!$GLOBALS['global_debug']):
	...
	//var_dump($man_show);
	$series_codes = array();
	$series_popular = array();
    $arSelect = Array("ID", "CODE","PROPERTY_POPULAR");
    ...
    while($ob = $res->GetNextElement())
    {
        ...
        $series_popular[$arFields['ID']] = $arFields['PROPERTY_POPULAR_VALUE'];
    }	
    // if ($USER->IsAdmin()){
    // 	echo "<pre>";
    ...
    foreach($arResult["ITEMS"] as $paramsPropId=>$prop)
    {
    	if($prop["CODE"]=="SERIES")
    	{
    		$seriesCode=$paramsPropId;
    	}
    }
    if($seriesCode>0)
    {
    	foreach($arResult["ITEMS"][$seriesCode]["VALUES"] as $propKey=>$propVal)
    	{
    		if(isset($series_popular[$propKey]) && $series_popular[$propKey]=="Y")
    		{
    			$seriesFirst[$propKey]=$propVal;
    		}
    		else
    		{
    			$seriesLast[$propKey]=$propVal;
    		}

    	}
    	$arResult["ITEMS"][$seriesCode]["POPULARS"]=count($seriesFirst);
    	
    	uasort($seriesFirst, function($a, $b) {
			return $a['ELEMENT_COUNT']<$b['ELEMENT_COUNT'];
		});
    	$arResultSeries=array_merge($seriesFirst,$seriesLast);
    }
	...
	<div class="selected-items">
		<div class="filter__content">
				...
				if ((isset($_GET['sort'])) && (isset($_GET['order']))){
	                $arResult["SEF_DEL_FILTER_URL"] .= '?sort='.$_GET['sort'].'&order='.$_GET['order'];
	            }
				?>
			...
			default:
				...
				<div class="filter__item">
					...
					<div class="filter__content filter__content--checkbox accordion-content accordionItem<?=$arItem["DISPLAY_EXPANDED"] != "Y" ? " is-collapsed" : " is-expanded"?>" id="accordion<?=$accordionId?>" aria-hidden="<?=$arItem["DISPLAY_EXPANDED"] == 'Y' ? 'false' : 'true'?>">
						...
						$i++):
							$val = $keys[$i];
							$ar = $isSeries?$arResultSeries[$val]:$arItem["VALUES"][$val];
							...
							if($isSeries)
							{
								$ar["CONTROL_ID"]=$ar["CONTROL_ID"]."f";
							}


							//if($GLOBALS['USER']->IsAdmin()){
								$arr_temp = explode('/', $_SERVER['REQUEST_URI']);
								if (in_array($ar["URL_ID"], $arr_temp))
									$ar["CHECKED"] = true;
							//}

							?>
							<div class="filter__checkbox filter-fast-search-checkbox filter-fast-search-checkbox-<?=$key?> <?=$isSeries?'first':''?>" data-key="<?=$key?>" data-fast-search="<?=$ar["VALUE"]?>">
								<input type="checkbox"
									id="<?=$ar["CONTROL_ID"]?>"
									name="<?=$ar["CONTROL_NAME"]?>"
									value="<?=$ar["HTML_VALUE"]?>"
						...
						<?endfor;
						if (!$showAll and $count >= $maxCount and $i < $count):?>
							<div class="dropdown filter__checkbox-link" onclick="<?=$isSeries?"$('.first').hide();":''?>">
								<input class="dropdown-open" type="checkbox" id="accordion<?=$accordionId?>_dropdown" data-key="<?=$key?>" data-id="accordion<?=$accordionId?>_dropdown_search" aria-hidden="true" hidden />
								<div class="dropdown-inner">
									<div class="filter__checkbox-link-content">
										<?
