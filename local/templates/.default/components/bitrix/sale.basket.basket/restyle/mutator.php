...
foreach ($this->basketItems as $row)
{
	...
	if ($rowData['MEASURE_RATIO'] != 1)
	{
		...
		{
			$rowData['PRICE'] = $price;
			$rowData['PRICE_FORMATED'] = customFormatPrice(CCurrencyLang::CurrencyFormat($price, $rowData['CURRENCY'], true));
		}
		...
		if ($fullPrice != $rowData['FULL_PRICE'])
		{
			...
			$rowData['FULL_PRICE_FORMATED'] = customFormatPrice(CCurrencyLang::CurrencyFormat($fullPrice, $rowData['CURRENCY'], true));
		}
		...
		if ($discountPrice != $rowData['DISCOUNT_PRICE'])
		{
			...
			$rowData['DISCOUNT_PRICE_FORMATED'] = customFormatPrice(CCurrencyLang::CurrencyFormat($discountPrice, $rowData['CURRENCY'], true));
		}
	}
