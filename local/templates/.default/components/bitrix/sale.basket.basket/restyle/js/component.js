;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		fillTotalBlocks: function()
		{
			...
			// var totalNodes = this.getEntities(this.getCacheNode(this.ids.basketRoot), 'basket-total-aside-block');

			// if (totalNodes && totalNodes.length)
			// {
			// 	var totalTemplate = this.getTemplate('basket-total-aside-template');
			// 	if (totalTemplate)
			// 	{
			// 		console.log(this.result.TOTAL_RENDER_DATA);
			// 		var totalRender = this.render(totalTemplate, this.result.TOTAL_RENDER_DATA);
			...
			$('#event-basket-changed').trigger('change');
		},
