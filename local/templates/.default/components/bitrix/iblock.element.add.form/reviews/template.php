...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<table class="data-table" style="width: 90%">
        ...
        <?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
            ...
            <? if ((int)$propertyID === 7215) { ?>
                <tr style="display: none">
            <? } else { ?>
                <tr>
            <? } ?>

                <td style="border: none;"><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></td>
					<td>
						<?
							...
							case "N":
								...
								?>
                                    ...
                                    <input type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]"
                                           ...
                                           class="field-hidden__product-id" style="display: none; visibility: hidden; opacity: 0;" /><br/>
                                <? } else { ?>
                                    <input type="text" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]"
                                           size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>"
                                           value="<?= $value ?>"/><br/>
