...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?if(strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
		...
		<div class="<?=$dropDownBlock?> bx-ui-sls-input-block">
			...
			<input id="<?=$arParams['INPUT_NAME']?>" type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="dropdown-field <?=$arParams['INPUT_CLASS']?>" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?> ..." />
