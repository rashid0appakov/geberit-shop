...
$jsParams = array(
	...
	"ajax_url" => "/ajax/form/complaint.php",
);?>
<form id="<?=$formId?>">
		...
		<div class="columns">
			<div class="column is-6">
				<div class="inputName">
					...
					<input id="<?=$orderInputId?>" name="inputNum" type="text" placeholder="123456" required>
				</div>
			</div>
			<div class="column is-6">
				<div class="inputFile">
					<div class="file-upload">
						<label class="inputFileFree">
							<input id="<?=$fileInputId?>" type="file" name="file">
							<span>Выбрать файл</span>
						</label>
					</div>
					<input type="text" class="filename" disabled required>
				</div>
			</div>
		</div>
		<div class="columns">
			<div class="column">
				<div class="inputText">
					<label for="<?=$textInputId?>">Описание притензии*</label>
					<textarea id="<?=$textInputId?>" name="text" cols="30" rows="10" required></textarea>
				</div>
			</div>
		</div>
	</div>
	<div class="showOnSubmit hide">
		<p class="formOkText">
			<strong id="<?=$submitNameSpanId?>"></strong>, спасибо! Ваше сообщение отправлено.
...
<script>
	window.feedbackComplaint = new JSFeedbackComplaint(<?=json_encode($jsParams)?>);
</script>