...
<? if (count($arResult['SIMILAR']) > 0 ) :?>
	<div class="carousel carousel-tabs goods__carousel-tabs hero" id="list-similars" data-nosnippet>
		<div class="container is-widescreen">
			<div class="level is-mobile carousel__title">
				<div class="level-left">
					<h2 class="is-size-3">Похожие товары</h2>
				</div>
			</div>
			<? if (count($arResult['SIMILAR']) > 1 && CClass::countItems($arResult['SIMILAR']) > 4):?>
			<section class="tabs goods__tabs">
				...
				<?php
					$i = 2;
					foreach($arResult['SIMILAR'] AS $key => &$sec ):?>
			...
			<div class="content">
				<div class="content tabs__content js-tabs-content2 active" id="tab-goods-add-1">
					<div class="swiper-container swiper-container-hit4 preview-products" id="<?=$carouselId?>">
					...
					<button id="<?=$leftArrowId?>" class="arrow left product-tabs-hit4-left"></button>
					<button id="<?=$rightArrowId?>" class="arrow right product-tabs-hit4-right custom-ajax-carousel-preload" data-id="<?=$carouselId?>" data-filter-key="ELEMENT" data-skip="2" data-pp="4" data-element="<?=$arResult['ID']?>" data-type="SIMILAR"></button>
		...
		if (count($arResult['SIMILAR']) > 1 && CClass::countItems($arResult['SIMILAR']) > 4):
			...
			foreach( $arResult['SIMILAR'] AS $key => &$sec):
				...
				<div class="content tabs__content js-tabs-content2" id="tab-goods_add_<?=$i;?>">
					...
					<button id="<?=$leftArrowId?>" class="arrow left product-tabs-hit4-left"></button>
					<button id="<?=$rightArrowId?>" class="arrow right product-tabs-hit4-right custom-ajax-carousel-preload" data-tab="<?=$key;?>" data-id="<?=$carouselId?>" data-filter-key="ELEMENT" data-skip="2" data-pp="4" data-element="<?=$arResult['ID']?>" data-type="SIMILAR"></button>
					<div class="finger-mobile"><?=GetMessage('CT_MOVE_FINGER')?></div>
					<?/*<div class="more-mobile">
					  <a href="#" class="btn is-primary is-outlined">Смотреть все</a>
					</div>*/?>
