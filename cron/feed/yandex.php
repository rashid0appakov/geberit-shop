...
$_SERVER['DOCUMENT_ROOT'] = realpath(dirname(__FILE__,2).'/..');
...
class YandexFeed
{
    ...
    protected function Load()
    {
        ...
        while ($row = $ob->GetNext())
        {
            ...
            $model = htmlspecialchars($row['PROPERTY_MODEL_VALUE']);
            ...
            if (!array_key_exists($elementId, $this->data['elements']))
            {
                ...
                $this->data['elements'][$elementId]['model'] = $model;
            }
        }
        unset($ob);
        ...
        while ($row = $ob->fetch())
        {
            ...
            $this->data['product_optimal_prices'][$productId] = CCatalogProduct::GetOptimalPrice($productId, 1, [], 'N', $arPrice, $this->SITE_ID);
        }
    }
    ...
    private function WriteFileOffers($h)
    {
        ...
        foreach ($this->data['elements'] as $elementId => $element)
        {
            ...
            $discountPrice = floatval($optimalPrice['RESULT_PRICE']['DISCOUNT_PRICE']);
            ...
            fwrite($h, "\t\t\t\t<model>" . $model_tag . "</model>" . PHP_EOL);
	...
	public function getDescription($sectionId, $arProp){
		...
		$arSkructure = array(
			...
			468 => array('Цвет', 'Форма', 'Монтаж', 'Организация смывающего потока', 'Система антивсплеск', 'Безободковый', 'Стилистика дизайна'),
			456 => array('Ширина', 'Форма', 'Стилистика дизайна', 'Монтаж', 'Цвет', 'Перелив', 'Тип раковины', 'Отверстия под смеситель'),
...
$feed = new YandexFeed();
$feed->Generate($_SERVER['DOCUMENT_ROOT'].'/feed/yandex.xml');